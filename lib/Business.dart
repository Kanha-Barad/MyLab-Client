...
class ClientBusiness extends StatefulWidget {
  ...
  @override
  State<ClientBusiness> createState() => _ClientBusinessState(0);
}

class _ClientBusinessState extends State<ClientBusiness> {
  int selectedPage;
  _ClientBusinessState(this.selectedPage);
  var selectedTabIndex = 0;
  String date = "";
//  DateTime selectedDate = DateTime.now();
  // String client_id = '';
  _selectClientBusinessDate(BuildContext context) async {
    ...
    setState(() {
      ...
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => ClientBusiness(globals.selectedClientid)
              //  ClientTestWise()
              ));
      // } else if (selectedTabIndex == 0) {
      //   Navigator.push(
      //       context, MaterialPageRoute(builder: (context) => ClientDeptWise()));
      // }
    });
  }
    ...
    final topAppBar = AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              _selectClientBusinessDate(context);
            },
          ),
        ],
        bottom: const TabBar(
        ...
        title: Row(
          children: [
            ...
            Container(
                width: 220,
                child:
                    Text(globals.clientName, style: TextStyle(fontSize: 16))),
          ],
        ));
...
class ClientTestWise extends StatefulWidget {
  ...
  int selectedPage;
  ClientTestWise(this.selectedPage);

  @override
  State<ClientTestWise> createState() => _ClientTestWiseState(0);
}
...
class _ClientTestWiseState extends State<ClientTestWise> {
  ...
  var selecteTodt = '';
  int selectedPage;
  _ClientTestWiseState(this.selectedPage);
  @override
  Widget build(BuildContext context) {
...
class ClientDeptWise extends StatefulWidget {
  ...
  int selectedPage;
  ClientDeptWise(this.selectedPage);

  @override
  State<ClientDeptWise> createState() => _ClientDeptWiseState(1);
}
...
class _ClientDeptWiseState extends State<ClientDeptWise> {
  ...
  var selecteTodt = '';
  int selectedPage;
  _ClientDeptWiseState(this.selectedPage);
...
class _salesMngrBillsState extends State<salesMngrBills> {
  ...
  Widget build(BuildContext context) {
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              ...
              if (snapshot.data!.isEmpty == true) {
                ...
                return clientBusinessListView(data);
              }
            } else if (snapshot.hasError) {
              return Text("${snapshot.error}");
            }
    ...
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Color(0xff123456),
          title: Text(globals.clientName, style: TextStyle(fontSize: 16))),
...
Widget clientBusinessListView(data) {
  int i = 0;
  return ListView.builder(
      itemCount: data.length,
      itemBuilder: (context, index) {
        return _buildClientBusinessCard(data[index], context, index);
      });
}

Widget _buildClientBusinessCard(data, BuildContext context, index) {
  return GestureDetector(
      onTap: () {
        globals.selectedPatientData = data;
        _showPicker(
